...
@Composable
fun ExpandedSearchListItemView(
    item: SearchItem,
    onButtonClicked: ((expanded: Boolean) -> Unit)? = null
) {
    Row(
        content = {
            Text(item.name, modifier = Modifier.weight(1F), fontWeight = FontWeight.Bold)
            Icon(
                imageVector = if (item.isExpanded) Icons.Default.ArrowDropUp else Icons.Default.ArrowDropDown,
                contentDescription = "name",
                tint = Color.Black, modifier = Modifier
                    .size(30.dp)
                    .clickable(onClick = {
                        onButtonClicked?.invoke(!item.isExpanded)
                    })
            )
        }, modifier = Modifier
            .fillMaxWidth()
